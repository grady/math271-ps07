...
\[\hat y = b_1x_1 + \cdots + b_nx_n = \sum_{i=1}^n b_ix_i = \vec b \cdot \vec x\]
There are $n$ coefficients $b_1,\ldots, b_n$ corresponding to model variables $x_1,\ldots,x_n$. Each of the model variables can be either a column of data directly, as in simple linear regression, or something computed from an observed variable, as in the categorical/dummy variable combinations above, or even just a constant, as in the intersept column.
